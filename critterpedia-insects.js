...
console.log("Captura de informações - Critterpedia Insects.")
...
const init = async () => {
    try{
        ...
        const pageUrl = 'https://nookipedia.com/wiki/Bug/New_Horizons';
        ...
        const locations = content_wrapper.querySelectorAll('tr > td:nth-child(5)').map((column) => column.textContent.trim() );
        const weather = content_wrapper.querySelectorAll('tr > td:nth-child(6)').map((column) => column.textContent.trim() );
        ...
        bugs_datas = names.map( (bug_name, index) => {
            ...
            const north_months = wrapper_north_months[index].querySelectorAll('span').map((month) => month.getAttribute('style').indexOf('color: #77AC3F') > -1 ? true : false);
            const south_months = wrapper_south_months[index].querySelectorAll('span').map((month) => month.getAttribute('style').indexOf('color: #77AC3F') > -1 ? true : false);
            return {
                name: bug_name,
                price: prices[index],
                icon: {
                    local: `./files/fish/${_to_slug(bug_name)}.png`,
                    remote: icons[index]
                },
        ...
        benchmark_start = performance.now();
        acnh.save_json('./files', 'critterpedia.insects.json', bugs_datas);
        benchmark_end = performance.now();
        acnh_util.print_log_message('Run 3: JSON salvo no arquivo critterpedia.insects.json', (benchmark_end - benchmark_start))
...
init().then(() => {
    console.log();
    console.log('Salvar inmagens - Critterpedia Insects.');
    benchmark_start = performance.now();
    bugs_datas.map( async (bug) => {
        const image_response = await fetch(bug.icon.remote, {method: 'GET'});
        const image_buffer = await image_response.arrayBuffer();
        const image_path = save_image('./files/insects', bug.name, image_buffer);
    });
    benchmark_end = performance.now();
